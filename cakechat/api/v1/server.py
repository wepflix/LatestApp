...
def get_model_response():
    ...
    try:
        ...
        return get_api_error_response('Malformed request Essam, no "{}" param was found'.format(e), 400, _logger)
    except ValueError as e:
        return get_api_error_response('Malformed request Essam: {}'.format(e), 400, _logger)

    emotion = params.get('emotion', DEFAULT_CONDITION)
    if emotion not in EMOTIONS_TYPES:
        return get_api_error_response(
            'Malformed request New Essam, emotion param "{}" is not in emotion list {}'.format(emotion, list(EMOTIONS_TYPES)),
            400, _logger)

    response = get_response(dialog_context, emotion)
    _logger.info('NEW RESPONSE PLEASE WORK: "{}" for context: {}; emotion "{}"'.format(response, dialog_context, emotion))
